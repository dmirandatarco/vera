    ...
    <div class="main-container container-fluid">
        ...
        <div class="breadcrumb-header justify-content-between">
            <div class="left-content">
                <span class="main-content-title mg-b-0 mg-b-lg-1">Listado de Ventas</span>
            </div>
                ...
                <div class="card">
                    <div class="card-body">
                                ...
                                <div class="row pb-4">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="form-group">
                                            <select name="searchResponsable" class="form-control form-select responsable"
                        ...
                        <div class="table-responsive">
                            <h3><b>Totales </b> {{number_format($ventas->where('estado',1)->sum('total'),2)}} </h3>
                            <table id="ventas" class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr class="fila-venta" style="{{$venta->estado == 0 ? 'background-color:#f4d2d1;':''}}" data-id="{{ $venta->id }}">
                                            <td>{{ ++$i }}</td>
                                            <td>{{ $venta->fecha }}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('venta.destroy', 'test') }}" method="POST" autocomplete="off">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
    ...
    <div class="modal fade" id="modalCobrar" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <div class="modal-body" >
                    <form action="{{ route('ventas.pagar', 'test') }}" method="POST" autocomplete="off" id="paymentForm">
                    {{csrf_field()}}
                    <div class="mb-3 ">
...
@php
    ...
    $canAnularVenta = Gate::check('venta.destroy');
    $canEditarVenta = Gate::check('venta.destroy');
@endphp
        ...
        function cargarDatosVenta(ventaId) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var anularVentaLink = '';
                    var canAnularVenta = {{ $canAnularVenta ? 'true' : 'false' }};
                    if(canAnularVenta && data.ver_pago && data.estado == 1){
                        anularVentaLink = '<a class="btn btn-primary mx-2 button-icon" href="#" data-bs-toggle="modal" data-bs-target="#EliminarMovimiento" id="btnAnular"><i class="pe-2 fa fa-times"></i>Anular</a>'
                    }
                    $('#btnAnularVentaBoton').html(anularVentaLink);
                    $('#btnAnular').attr('data-id',data.id);
                    var editarVentaLink = '';
                    var nombreDocumento = data.documento.nombre;
                    var canEditarVenta = {{ $canEditarVenta ? 'true' : 'false' }};
                    if(canEditarVenta && data.ver_pago && data.estado == 1 && nombreDocumento == "NOTA DE VENTA" && data.tipo == 1 || canEditarVenta && data.ver_pago && data.estado == 1 && nombreDocumento == "NOTA DE VENTA" && data.tipo == 0 && data.trabajos.length <= 1){
                        editarVentaLink = '<a class="btn btn-primary mx-2 button-icon" href="{{url('ventas/')}}/'+data.id+'/edit" id="btnEditar"><i class="pe-2 fa fa-edit"></i>Editar</a>'
                    }
                    $('#btnEditarVentaBoton').html(editarVentaLink);
                },
